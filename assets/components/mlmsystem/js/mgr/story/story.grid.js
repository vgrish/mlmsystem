mlmsystem.grid.Story = function(config) {
    config = config || {};
            ...
            notifyDrop: function(dd, e, data) {
                ...
                if (target != source) {
                    ...
                    MODx.Ajax.request({
                        ...
                        params: {
                            action: config.action || 'mgr/story/sort',
                            source: source,
                            target: target
                        },
                        listeners: {
    ...
    Ext.applyIf(config, {
        id: 'mlmsystem-grid-story',
        url: mlmsystem.config.connector_url,
        baseParams: {
            action: 'mgr/story/getlist',
            class: config.class || ''
        },
        save_action: 'mgr/story/updatefromgrid',
        autosave: true,
        ...
        stateful: true,
        stateId: 'mlmsystem-grid-story-state'

    });
    mlmsystem.grid.Story.superclass.constructor.call(this, config);
    this.getStore().sortInfo = {
        field: 'id',
        direction: 'DESC'
    };
};
Ext.extend(mlmsystem.grid.Story, MODx.grid.Grid, {
    windows: {},
            ...
            listeners: {
                success: {
                    fn: function(r) {
                        var record = r.object;
                        var w = MODx.load({
                            xtype: 'mlmsystem-story-window-view',
...
Ext.reg('mlmsystem-grid-story', mlmsystem.grid.Story);
