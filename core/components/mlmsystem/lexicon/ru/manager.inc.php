<?php

/* header */
$_lang['mlmsystem_header_id'] = '<i class="fa fa-sort-numeric-desc"></i>';
$_lang['mlmsystem_header_resource'] = '<i class="fa fa-folder-o"></i>';
$_lang['mlmsystem_header_file'] = '<i class="fa fa-file"></i>';
$_lang['mlmsystem_header_type'] = '<i class="fa fa-info"></i>';
$_lang['mlmsystem_header_method'] = '<i class="fa fa-info"></i>';
$_lang['mlmsystem_header_createdon'] = '<i class="fa fa-calendar"></i>';
$_lang['mlmsystem_header_updatedon'] = '<i class="fa fa-calendar"></i>';
$_lang['mlmsystem_header_createdby'] = '<i class="fa fa-user"></i>';
$_lang['mlmsystem_header_actions'] = '<i class="fa fa-cogs"></i>';
$_lang['mlmsystem_header_name'] = '<i class="fa fa-info"></i>';
$_lang['mlmsystem_header_row'] = '<i class="fa fa-file-code-o"></i>';
$_lang['mlmsystem_header_wrapper'] = '<i class="fa fa-file-code-o"></i>';
$_lang['mlmsystem_header_format'] = '<i class="fa fa-expand"></i>';
$_lang['mlmsystem_header_username'] = '<i class="fa fa-user green"></i>';
$_lang['mlmsystem_header_email'] = '<i class="fa fa-envelope "></i>';
$_lang['mlmsystem_header_tax'] = '<i class="fa fa-money red"></i>';
$_lang['mlmsystem_header_sum'] = '<i class="fa fa-money green"></i>';
$_lang['mlmsystem_header_cost'] = '<i class="fa fa-money"></i>';
$_lang['mlmsystem_header_class'] = '<i class="fa fa-file-o"></i>';
$_lang['mlmsystem_header_num'] = '<i class="fa fa-qrcode"></i>';
$_lang['mlmsystem_header_sender'] = '<i class="fa fa-user red"></i>';
$_lang['mlmsystem_header_recipient'] = '<i class="fa fa-user green"></i>';
$_lang['mlmsystem_header_status'] = '<i class="fa fa-heartbeat"></i>';
$_lang['mlmsystem_header_payment'] = '<i class="fa fa-ticket"></i>';
$_lang['mlmsystem_header_color'] = '<i class="fa fa-adjust"></i>';
$_lang['mlmsystem_header_context'] = '<i class="fa fa-globe"></i>';
$_lang['mlmsystem_header_balance'] = '<i class="fa fa-money"></i>';
$_lang['mlmsystem_header_incoming'] = '<i class="fa fa-money green"></i>';
$_lang['mlmsystem_header_outcoming'] = '<i class="fa fa-money red"></i>';
$_lang['mlmsystem_header_handler'] = '<i class="fa fa-file-code-o"></i>';


/* tooltip */
$_lang['mlmsystem_tooltip_id'] = 'Id';
$_lang['mlmsystem_tooltip_resource'] = 'Ресурс';
$_lang['mlmsystem_tooltip_file'] = 'Файл';
$_lang['mlmsystem_tooltip_type'] = 'Тип';
$_lang['mlmsystem_tooltip_method'] = 'Метод';
$_lang['mlmsystem_tooltip_createdon'] = 'Дата создания';
$_lang['mlmsystem_tooltip_updatedon'] = 'Дата обновления';
$_lang['mlmsystem_tooltip_createdby'] = 'Создатель';
$_lang['mlmsystem_tooltip_actions'] = 'Действия';
$_lang['mlmsystem_tooltip_name'] = 'Имя';
$_lang['mlmsystem_tooltip_row'] = 'Чанк оформления';
$_lang['mlmsystem_tooltip_wrapper'] = 'Чанк-обёртка';
$_lang['mlmsystem_tooltip_format'] = 'Формат';
$_lang['mlmsystem_tooltip_username'] = 'Пользователь';
$_lang['mlmsystem_tooltip_email'] = 'Почта';
$_lang['mlmsystem_tooltip_tax'] = 'Комиссия';
$_lang['mlmsystem_tooltip_sum'] = 'Сумма';
$_lang['mlmsystem_tooltip_cost'] = 'Стоимость';
$_lang['mlmsystem_tooltip_class'] = 'Класс-обработчик';
$_lang['mlmsystem_tooltip_num'] = 'Номер';
$_lang['mlmsystem_tooltip_sender'] = 'Отправитель';
$_lang['mlmsystem_tooltip_recipient'] = 'Получатель';
$_lang['mlmsystem_tooltip_status'] = 'Статус';
$_lang['mlmsystem_tooltip_payment'] = 'Оплата';
$_lang['mlmsystem_tooltip_color'] = 'Цвет';
$_lang['mlmsystem_tooltip_context'] = 'Контекст';
$_lang['mlmsystem_tooltip_balance'] = 'Баланс';
$_lang['mlmsystem_tooltip_incoming'] = 'Приход';
$_lang['mlmsystem_tooltip_outcoming'] = 'Расход';
$_lang['mlmsystem_tooltip_handler'] = 'Обработчик';


/* fields */
$_lang['mlmsystem_name'] = 'Имя';
$_lang['mlmsystem_email'] = 'Почта';
$_lang['mlmsystem_notify'] = 'Уведомить';
$_lang['mlmsystem_chunk'] = 'Шаблон';
$_lang['mlmsystem_sum'] = 'Сумма';
$_lang['mlmsystem_cost'] = 'Стоимость';
$_lang['mlmsystem_sender'] = 'Отправитель';
$_lang['mlmsystem_recipient'] = 'Получатель';
$_lang['mlmsystem_comment'] = 'Комментарий';
$_lang['mlmsystem_description'] = 'Описание';
$_lang['mlmsystem_purpose'] = 'Назначение';
$_lang['mlmsystem_payment'] = 'Способ оплаты';
$_lang['mlmsystem_callbacks'] = 'Обратный вызов';
$_lang['mlmsystem_active'] = 'Включен';
$_lang['mlmsystem_class'] = 'Класс-обработчик';
$_lang['mlmsystem_tax'] = 'Комиссия';
$_lang['mlmsystem_type'] = 'Тип';
$_lang['mlmsystem_method'] = 'Метод';
$_lang['mlmsystem_all'] = 'Все';
$_lang['mlmsystem_color'] = 'Цвет';
$_lang['mlmsystem_context'] = 'Контекст';
$_lang['mlmsystem_status'] = 'Статус';
$_lang['mlmsystem_logo'] = 'Логотип';
$_lang['mlmsystem_money_unit'] = 'p.';
$_lang['mlmsystem_balance'] = 'Баланс';
$_lang['mlmsystem_incoming'] = 'Приход';
$_lang['mlmsystem_outcoming'] = 'Расход';
$_lang['mlmsystem_handler'] = 'Обработчик';
$_lang['mlmsystem_user'] = 'Пользователь';
$_lang['mlmsystem_num'] = 'Номер';
$_lang['mlmsystem_email_user'] = 'Письмо клиенту';
$_lang['mlmsystem_email_manager'] = 'Письмо менеджеру';
$_lang['mlmsystem_subject_user'] = 'Тема письма клиенту';
$_lang['mlmsystem_subject_manager'] = 'Тема письма менеджеру';
$_lang['mlmsystem_body_user'] = 'Чанк письма клиенту';
$_lang['mlmsystem_body_manager'] = 'Чанк письма менеджеру';
$_lang['mlmsystem_status_final'] = 'Итоговый';
$_lang['mlmsystem_status_fixed'] = 'Фиксирует';


/* action */
$_lang['mlmsystem_action_active'] = 'Включить';
$_lang['mlmsystem_action_inactive'] = 'Выключить';
$_lang['mlmsystem_action_remove'] = 'Удалить';
$_lang['mlmsystem_action_create'] = 'Создать';
$_lang['mlmsystem_action_update'] = 'Обновить';
$_lang['mlmsystem_action_edit'] = 'Редактировать';
$_lang['mlmsystem_action_notice'] = 'Уведомить';
$_lang['mlmsystem_action_correct_balance'] = 'Корректировать баланс';


/* confirm */
$_lang['mlmsystem_confirm_remove'] = 'Вы уверены, что хотите удалить это?';
$_lang['mlmsystem_confirm_send'] = 'Вы уверены, что хотите отправить?';
